## 1.2.2
* Added the option to disable autofill

## 1.2.1
* Added new certificate for ssl pinning
* Switched tokenize and save cvv to new domain
* Added autofill to card form fields

## 1.2.0
* Added card installments for Romania market
* Switched tokenize and save cvv endpoints
* Added SDK version to tokenize query
* Compatibility with Dart 3.11, 3.12

## 1.1.0
* Min Dart version 3.8
* Compatibility with Dart 3.8, 3.9, 3.10
* Do not require PaymentContact in ApplePay
* Add Lithuanian language

## 1.0.7
* Fixed double base64 Apple Pay token

## 1.0.6
* Upgraded webview_flutter package

## 1.0.5
* Added new certificate for ssl pinning
* Added new address for Sandbox
* Relaxed int version

## 1.0.4
* Min Dart version 3.4
* Removed mastercard installments
* Dependencies update

## 1.0.3
* Compatibility with Dart 3.4
* Dependencies update

## 1.0.2
* Cleared translations
* Added new languages - Bulgarian, Greek, Croatian, Italian, Russian, Slovak
* Remove payu_about
* Dependencies update

## 1.0.1
* Update PayU payment terms URLs
* Dependencies update
